(ns ovh.main
  "Kotodama WASM entrypoint for the Ovh clean-room actor (L4) — Clojure port.
  ...
  facts (`ovh.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(def entity-specs
  [{:entity "Project"   :plural "projects"   :id-prefix "ovh_pro"
    :fields [:name :region :billingId]              :required [:name :region]
    :coerce {}                       :refs {}}
   {:entity "ComputeInstance" :plural "computeinstances" :id-prefix "ovh_com"
    :fields [:projectId :name :machineType :state :ipAddress] :required [:name :machineType]
    :coerce {}                       :refs {:projectId "Project"}}
   {:entity "Volume"    :plural "volumes"    :id-prefix "ovh_vol"
    :fields [:projectId :sizeGb :type :attachedTo]  :required [:sizeGb :type]
    :coerce {:sizeGb :int}           :refs {:projectId "Project"}}
   {:entity "Bucket"    :plural "buckets"    :id-prefix "ovh_buc"
    :fields [:projectId :name :region :public]     :required [:name :region]
    :coerce {:public :bool}          :refs {:projectId "Project"}}
   {:entity "Network"   :plural "networks"   :id-prefix "ovh_net"
    :fields [:projectId :cidr :region]             :required [:cidr :region]
    :coerce {}                       :refs {:projectId "Project"}}
   {:entity "IamRole"   :plural "iamroles"   :id-prefix "ovh_iam"
    :fields [:projectId :name :policy]             :required [:name :policy]
    :coerce {}                       :refs {:projectId "Project"}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"ovh.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "ovh-compat" :tier tier :entities entities} 200])
